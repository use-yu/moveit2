...
- 订阅实际左臂/右臂/升降/腰部/头部状态话题
  -> 合成为 /g01/joint_states（sensor_msgs/JointState）

约束：
- 五部分都收到新鲜状态后才发布 /g01/joint_states，避免头部旧值参与视觉转换。
...
class HwTopics:
    ...
    # Lift/waist/head state topics use MotorRuntimeInfo.
...
def _load_topics() -> HwTopics:
    return HwTopics(
        ...
        head_state_topic="/driver_report/camera_motor_info",
    )
...
class G01Comm(Node):
    def __init__(self) -> None:
        ...
        self.create_subscription(
            MotorRuntimeInfo, self._topics.head_state_topic, self._on_head_state, 10
        )

        # 周期发布 joint_states；只有五部分都收到过才开始发布
        ...
        self.get_logger().info(
            f"Bridge: arm states(Float32MultiArray) + lift/waist/head states(MotorRuntimeInfo)"
    ...
    def _on_head_state(self, msg: MotorRuntimeInfo) -> None:
        position = float(msg.position)
        if not math.isfinite(position):
            return
        self._pos[HEAD_JOINT] = position  # 与腰部一样，反馈单位为 rad。
        self._touch_group("head")

    def _publish_joint_states(self) -> None:
        if not (self._left_rx and self._right_rx and self._lift_rx and self._waist_rx and self._head_rx):
            return
        ...
        if self._part_timeout_sec > 0.0:
            ...
            if (
                ...
                or now_ns - self._head_last_ns > timeout_ns
            ):
                return
